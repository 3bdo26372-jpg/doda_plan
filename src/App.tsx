...
function ExerciseMedia({ exercise,index,onOpen,language }: { exercise:Exercise; index:number; onOpen:(exercise:Exercise)=>void; language:Language }) {
  ...
  return <button className="exercise-gif interactive" onClick={()=>onOpen(exercise)} aria-label={ar?`فتح عرض ${name}`:`Open ${name} GIF demonstration`}>
    {exercise.gif?<img src={exercise.gif} alt={ar?`عرض حركة ${name}`:`${name} movement demonstration`}/>:<div className="gif-placeholder"><Dumbbell/><span>{ar?'العرض قريباً':'GIF demo coming soon'}</span></div>}
...
function App() {
  ...
  return <div className="app-shell routed-app simplified-app" dir={ar?'rtl':'ltr'}>
    <header className="site-header product-header">
      <button className="brand" onClick={()=>navigate('A')} aria-label={ar?'الانتقال إلى اليوم A':'Go to Day A'}><span className="brand-mark"><Heart/></span><span><strong>{ar?'دودا':'DODA'}</strong><small>{ar?'قوة وازدهار':'FIT & FLOURISH'}</small></span></button>
