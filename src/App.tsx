...
const englishInstructions: Record<string, Exercise['instruction']> = {
  ...
  latPulldownB:{steps:['Choose a closer or neutral grip if available.','Lock thighs under the pad and sit tall.','Hold without bending your wrists.','Pull the handle toward your upper chest.','Return slowly and let shoulders stretch comfortably.'],mistakes:['❌ Do not pull toward your stomach.','❌ Do not swing your body.','❌ Do not let elbows drift too far forward.','❌ Do not let the weight yank your shoulders.'],tips:['The different grip changes the feel, not the goal.','Try neutral grip if it feels better on shoulders.','Keep your neck long.','Focus on your back, not hand strength.'],breathing:['Inhale as the handle rises.','Exhale as you pull down.'],target:{primary:'Lats',secondary:'Upper back and biceps'},difficulty:'Beginner'},
...
const englishCues: Record<string, string> = {
  ...
  latPulldownB:'Use the different grip to pull toward your upper chest with your elbows, not just your hands.',
...
function App() {
  ...
  const [language,setLanguage]=useState<Language>(()=>localStorage.getItem('doda-language')==='ar'?'ar':'en')
  ...
  useEffect(()=>{document.documentElement.lang=language;document.documentElement.dir=ar?'rtl':'ltr';localStorage.setItem('doda-language',language)},[language,ar])
  ...
  return <div className="app-shell routed-app simplified-app" dir={ar?'rtl':'ltr'}>
    <header className="site-header product-header">
      <button className="brand" onClick={()=>navigate('1')} aria-label={ar?'الانتقال إلى اليوم 1':'Go to Day 1'}><span className="brand-mark"><Heart/></span><span><strong>{ar?'دودا':'DODA'}</strong><small>{ar?'قوة وازدهار':'FIT & FLOURISH'}</small></span></button>
